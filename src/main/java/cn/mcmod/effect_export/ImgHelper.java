...
public class ImgHelper {
    ...
    private void getEffect() throws IOException {
        for (StatusEffect effect : Registry.STATUS_EFFECT) {
            var effectName = Registry.STATUS_EFFECT.getId(effect);
            if (!effectName.getNamespace().equals(modid)) continue;
            String name = effect.getName().getString();
            String englishname = langMap.getOrDefault(effect.getTranslationKey(), effect.getName().getString());
    ...
    private String getIcon(int size, Identifier location) throws IOException {
